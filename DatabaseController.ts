...
export class DatabaseController {
	...
	async loadDatabase(): Promise<void> {
		try {
			let databaseFile = this.app.vault.getAbstractFileByPath(this.databasePath) as TFile;
			...
			if (!databaseFile) {
				...
				await this.app.vault.create(this.databasePath, defaultContent);
				databaseFile = this.app.vault.getAbstractFileByPath(this.databasePath) as TFile;
			}
	...
	private async getUnprocessedFiles(): Promise<TFile[]> {
        ...
        return allFiles.filter(file => 
            ...
            !this.database.notesMetaData.some(note => note.fileName === file.path));
    }
	...
	async processFiles(files: TFile[]): Promise<void> {
        for (const file of files) {
            ...
            const taggedInfos = this.extractTaggedInfoFromFile(file,this.tagName);

            // Update or add note metadata
            const existingNoteIndex = this.database.notesMetaData.findIndex(note => note.fileName === file.name);
            if (existingNoteIndex !== -1) {
                this.database.notesMetaData[existingNoteIndex].taggedInfo = await taggedInfos;
            } else {
	...
	async clearDatabaseForNewSettings(newPath: string, newTagName: string): Promise<void> {
        this.database = {
            lastUpdateCheck: 0,
            notesMetaData: [],
            path: newPath
